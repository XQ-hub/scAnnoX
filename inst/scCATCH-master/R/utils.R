...
.get_pvalue <- function(ndata1, ndata2) {
    ...
    for (k in 1:nrow(ndata1)) {
        ...
        clu_marker_pvalue[k] <- as.numeric(stats::wilcox.test(genedata1, genedata2, alternative = "greater", paired = FALSE, exact = FALSE)$p.value)
    }
...
.get_marker_scCATCH1 <- function(ndata, meta, cluster, clu_num, clu_pair, cell_min_pct, logfc, pvalue, comp_cluster, verbose) {
    ...
    pb <- progress::progress_bar$new(
        format = "[:bar] Finished::percent Time :elapsedfull", total = length(clu_num), clear = FALSE,
...
.get_clu_ann_subtype <- function(clu_ann, clu_ann_cellname, celltype, celltype_score, clu_marker, PMID) {
    ...
    res <- list(
        cellsubtype1 = cellsubtype1, cellsubtype2 = cellsubtype2, cellsubtype3 = cellsubtype3, celltype = celltype,
